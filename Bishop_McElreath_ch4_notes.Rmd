...
NB This is a Notebook file. You can run each chunk by just clicking on the little green arrow in top right of chunk window. And you can view the output by clicking the Preview option on the bar at the top. This shows the html output in a separate pane.
Also allows you to include mathematical notation in the text.
...
See also: http://doingbayesiandataanalysis.blogspot.com/2012/05/graphical-model-diagrams-in-doing.html  
Nice graphical models
...
We'll first redefine the original m4.3
...
weight.seq <- seq(from=25,to=70,by=1) #there are 46 values of weight.seq
...
weight.seqb <- seq( from=25 , to=70 , by=5 ) #25,30,35 ...etc
weight.seqb
myout <- sapply(weight.seqb,myfunc) #yes, this doubles weight and adds 3!
myout
...
#Summarise simulated heights; find probability interval
...
First redo original:
```{r modelm4.3}
d <- Howell1 #I have changed name from d to myd, as I find single character names 
...
#bolt results together so we can compare
rbind(summary(m4.3),summary(m4.3b))
rbind(vcov(m4.3),vcov(m4.3b))
```

Table shows first results from original model, then from model with no xbar.

The estimates for variable a change, but the other estimates remain same. 
Variances are now more variable in magnitude - presumably because not centred.

Now compare posterior predictions.

```{r compare.postpreds}
sim.height1 <- sim(m4.3,data=list(weight=weight.seq),n=1e4)
sim.height2 <- sim(m4.3b,data=list(weight=weight.seq),n=1e4)
height.PI1<-apply(sim.height,2,PI,prob=.89)
height.PI2<-apply(sim.height2,2,PI,prob=.89)

dens(sim.height1)
dens(sim.height2,add=T,col='blue')
```

Hmm. They seem the same.
Not sure if that's right, though it does seem as if just rescaling a variable should not alter the likelihood function.  

Is the point that centring is just done to help with model fitting?

## 4M8.
I have already played about with splines so won't do this one. 
I need to be convinced that it is worth spending mental energy on splines!!

## 4H1

Find expected heights and 89% interval for individuals from the model

```{r do.predict}
mydf <- data.frame(matrix(NA,nrow=5,ncol=5))
colnames(mydf)<-c('ID','weight','pred.ht','PIlo','PIhi')
mydf$weight<-c(46.95,43.72,64.78,32.59,54.63)
mydf$ID<-1:5
weight.list<-seq(32,65) #range to span values to estimate
nusim <- sim( m4.3 ,data=list(weight=mydf$weight))
dim(nusim) #1000 rows for each weight


mydf$pred.ht <- colMeans(nusim)
#Go through each value to find PI and put in data frame.
# I suspect this bit could be done without the loop....
for(i in 1:5){
  mydf[i,4:5]<-PI(nusim[,i],.89)
}

```
## 4H2
Select out rows in Howell1 data with ages below 18.
a) Fit linear regression to the data using quap
...
Child grows 27 cm for every 10 units of increase in weight.

b) Plot the raw data, with height on vertical axis and wt on horizontal. Superimpose MAP regression line and 89% interval for mean.
Also superimpose 89% interval for predicted heights.

```{r plotchild}
plot(d3$weight,d3$height)
post <- extract.samples( m4.3c )
a_map <- mean(post$a) 
b_map <- mean(post$b) 


curve( a_map + b_map*x,from = 5,to=50,add=T) 
wtrange<-5:50
mywtdf <- data.frame(wtrange)
colnames(mywtdf)<-'weight'
mu <- link(m4.3c,data=mywtdf) #one column for each value of wtrange

#this is what I've been struggling with as I find it much easier to work with transposed matrix, where you can then see weight increasing with each row.

#But once you understand this is a matrix where there's a value for each sample (rows) for each weight value (columns) it is easier to work out what is going on. But here, sample is defined as values of weight, rather than the true data.

plot(height~weight,d3,type='n')
for(i in 1:20){
  points(wtrange,mu[i,],col=col.alpha(rangi2,0.1))
}

#summarise distribution of mu
mu.mean <- apply(mu,2,mean)  #OK. this is same as colMeans(mu). The 2 indicates mean is computed over 2nd dimension (columns)
mu.PI <- apply(mu,2,PI,prob=.89) #gives pair of values for each value of weight
```

I think that now if I plot (wtrange,mu.mean) this should be identical to the 'curve' result above. Let's see.
Starting over with original code:
```{r exercise4H2}
plot(d3$weight,d3$height,xlim=c(5,50))
post <- extract.samples( m4.3c )
a_map <- mean(post$a) 
b_map <- mean(post$b) 
mu <- link(m4.3c,data=mywtdf) #one column for each value of wtrange
...
wtrange<-5:50
lines(wtrange,mu.mean,col='red') 
curve( a_map + b_map*x,from = 5,to=50,add=T) 
shade(mu.PI,wtrange)
```
I struggled with this for a long time, only to find I had specified link function for m4.3 rather than m4.3c.

But with right link function, yes, this gives what we want.

We now need to add the shaded area for predicted heights.
For this we need to use sim rather than link to generate values.

```{r shadepredictions}
mysim <- sim(m4.3c,data=mywtdf)
#check: each column corresponds to data simulated for one of the weight values
meanvals <- colMeans(mysim) #can see these increasing systematically

meanvals.PI <- apply(mysim,2,PI,prob=.89) #gives pair of values for each value of weight

#now just repeat the plot with this added
plot(d3$weight,d3$height,xlim=c(5,50))
lines(wtrange,mu.mean,col='red') 
curve( a_map + b_map*x,from = 5,to=50,add=T) 
shade(mu.PI,wtrange)
shade(meanvals.PI,wtrange)
```

Yes!

This reinforces the fact that there are parallels between link and sim.
Both of them generate a lot of simulated data (default 1000 values).
If you don't specify a dataset, the simulation will use the raw data you started with, and each column will correspond to a datapoint for which new data is simulated.
If you do specify a dataset, the simulation will use that. So if you want to generate data for a given range of x, you can just make a little dataframe with values of x, and it will use those. This has to have the key x-variable (in this case weight) with the same name as in the original dataset.

With link, the generated data are estimates of the parameter values, e.g. mu in this case (derived from a and b), and the probability interval around this is just reflecting precision of estimate of mu.

With sim, the generated data are simulated values of weight in the population, which use the sigma as well as the mu value. This reflects the fact that individual people will vary in mu. There will always be a bigger spread around esimates that use sim, rather than those based on link (I think!).

Things I found helpful in getting to grips with this:
Looking at dimensions of output from link and sim, and then looking at some of the values as well.  
Making a mistake! I spent some time utterly baffled as to why my plot from the link function didn't sit on top of the value specified in 'curve', but that made me look at the mu I'd got from the link function and it did not make sense (apart from anything else, it had wrong dimensions) - being forced to really look at what is in the variables is good to track down errors.  
I'm not used to using apply, even though I realise I should use it, as it is much more efficient than using loops. But realising that 
mu.mean <- apply(mu,2,mean) is same as colMeans(mu) was a leap forward in understanding, and I could then generalise that knowledge a bit.


Getting back to the exercise, the main problem with the plot is that data has curvilinear relation between wt and ht, and this is linear.
A quadratic function might be better.  

## Exercise 4H3
Model relation of ht to log weight, using whole dataset. Can I interpret the estimates? (see further below - I did not fit the model as described in the exercise, though all looks good!)

```{r ex4H3a}
d <- Howell1 
d$logwt <- log(d$weight)
plot(d$weight,d$logwt,main='Demo of how log transform works') #illustrating impact of log transform
#Log transform flattens out larger values
plot(d$logwt,d$height,main='Log wt x height, whole sample')
m4.3d <- quap( 
alist(
height ~ dnorm( mu , sigma ) ,
mu <- a + b* logwt ,
...
Interpretation: negative intercept is OK, because logwt is never zero ,and b is large, so will always end up adding the b-term which will give a positive value.

b) start with a plot of height x weight and then superimpose things on the plot.

```{r htwtplots}
plot(height~weight,data=d,pch=4,cex=.5) #pch is cross, cex is size; so small crosses
wtrange<-5:65 #need larger range with adults included
mywtdf <- data.frame(wtrange)
colnames(mywtdf)<-'weight'
mywtdf$logwt <- log(mywtdf$weight)
mu <- link(m4.3d,data=mywtdf)
mumeans <- apply(mu,2,mean)

mu.PI <- apply(mu,2,PI,.97)
mysim <- sim(m4.3d,data=mywtdf)
meanvals.PI <- apply(mysim,2,PI,prob=.97) #gives pair of values for each value of weight
lines(wtrange,mumeans, col ='red',lty=2) #made line dashed in order to see shaded area around it, as it is very tight.
shade(mu.PI,wtrange,col=col.alpha(rangi2,0.5))
shade(meanvals.PI,wtrange)

```

Can also check 97% interval makes sense: with 544 cases, would expect around 15 to be outside this interval - that is the case.

Oh, just realised I did not follow instructions re model to be fitted! Here is model specification
$$hi \sim Normal(\mu_i, \sigma)$$
...
$$\sigma \sim Uniform(0, 50)$$
So we'll create this as revised model4.3e
...
This is virtually identical to previous model, even though the prior for a in my model d was considerably smaller. Only other difference was that instead of precomputing logweight as I did for model 4.3d, here it is computed within quap
...
Can you modify the prior distributions of $$\alpha, \beta_1, and  \beta_2$$ so that the prior predictions stay within the biologically reasonable outcome space? That is to say: Do not try to fit the data by hand. But do try to keep the curves consistent with what you know about height and weight, before seeing these exact data.

see p 110-111 of book.
First we'll just repeat the model to remind ourselves of it
...
a ~ dnorm( 178, 20 ) ,
b1 ~ dlnorm( 0 , 1 ) ,
b2 ~ dnorm( 0 , 1 ) , #don't use log transform for quadratic term
sigma ~ dunif( 0 , 50 )
) ,
data=d )
precis(m4.5)
...
```
```{r plotm4.5}
weight.seq <- seq(from =-2.2, to =2, length.out=30) #these are like z-scores so range is -2.2 to 2 to capture most values
pred_dat <- list(weight_s=weight.seq, weight_s2 = weight.seq^2)
#for the link function we need 2 predictors for weight, for b1 and b2
#b1 is just the standardized weight sequence, but b2 is squared

mu <- link(m4.5,data=pred_dat)
dim(mu) #we have 1000 rows and 30 cols ; cols correspond to b1/b2 values
mu.mean <- apply(mu,2,mean) #mean prior predicted for each b1/b2 value
mu.PI <- apply (mu,2, PI, prob=.89)

sim.height <- sim(m4.5,data=pred_dat) #as before, direct parallel with link, except this time we simulate data, rather than the mu values
height.PI <- apply(sim.height, 2, PI, prob=.89)

plot(height ~ weight_s,d,col=col.alpha(rangi2,.5),main='Quadratic fit m4.5')
lines(weight.seq,mu.mean)
shade(mu.PI,weight.seq)
...
#Now compute height distribution by sampling from priors (based on code R4.14, 4.38)
N <- 500
sample_a <- rnorm( N , 178 , 50 ) #just sample N random values from prior a
sample_b1 <- rlnorm( N ,0,1 ) #just sample N random values from prior b1
sample_b2 <- rnorm( N ,0,1 ) #just sample N random values from prior b2

sample_sigma <- runif( N, 0 , 50 )#just sample 10000 random values from prior sigma (?needed here)

wtrange <- seq(from = -2, to = 2, length.out=30) #range for standardized wts

#Just checking what predicted hts are like for these 100 simulated values

predht.df <- data.frame(matrix(NA,nrow=N,ncol=30))
for (i in 1:N){
  predht.df[i,]<-sample_a[i]+sample_b1[i]*wtrange+sample_b2[i]*wtrange^2
}
meanpreds<-colMeans(predht.df)
plot(wtrange,meanpreds,ylim=c(80,200),col='red',pch=15)

#Looking at individual plots from predht.df.
#They are more nonlinear, but seem to cancel each other out.
for (i in 1:20){
  lines(wtrange,predht.df[i,])
}


```

I've played around a fair bit with the priors.
Was surprised that with the values from the example, function looks remarkably linear, so altered things to make it less so.
Things that are interesting to explore are relative size of SD of a, relative to variation in b1 and b2.
Also explore making b2 lognorm as well as b1.
Then you get curvilinear, but with a dip at mean - makes sense because mean is zero, whereas squared values to either size are larger.

After moving on to chapter 5, realise I had neglected the 'extract prior' command, which might be useful here.
```{r more4h4}
mypriors <- extract.prior(m4.5)

```
Hmm, I think it's just an easy way to get the sets of values for sample_a, sample_b1 etc, as it generates a list which has values for all parameters.

But in ch 5, p 126, M shows you can then use those values with the link function.

So this is a much easier way to do what I tried above in prior h4.4
```{r yetmore4h4}

mu <- link(m4.5,post=mypriors,data=list(weight_s=wtrange,weight_s2=wtrange^2))

mu.means<-colMeans(mu)
plot(wtrange,mu.means,ylim=c(80,200),col='red',pch=15)
