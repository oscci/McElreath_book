...
#Now plot height distribution from prior (code R4.14)
...
```{r code4.16}
#below I changed d2 to myd2 in line 255
...
```
## Diversion for exercises

4E1 - likelihood is the y function: the one whose terms are variables - the other element of the model specify what values the variables take.

4E2 - I *think* the answer is 2 : mean and sd
...
? So this is something like the formula at bottom of p 80?
OOH THIS IS HARD
I'm going to have to use M and S for observed values and m and s for the mu and sigma
...
4E4 - line 2 looks familiar as a linear model to me!
...
#sample_sigma <- exp(1) #?  no variation in sigma? - should we take exp of a runif?
#found this on simulation of random exponential https://rpubs.com/rsshalini/109890
...
Im very confused here, but my effort is below
```{r task4M2}
#based on rcode 4.31
...
Still feeling hazy so will make a toy example.

```{r toylists_ignore this}
#Just for posterity - my failed attempt to understand alist in this chunk. 

#I am OK with having a formula specified for use in lm
myf <- formula(height~weight)
lm(myf,data=d2)

#But list doesnt work like that
# This doesn't work
myalist <- alist(height~weight)
lm(myalist,data=d2) 

# How about testing with mvrnorm, which needs several arguments
...
mvrnorm(mlist)
#Nope. This gets: 
#Error in mvrnorm(mylist) : argument "mu" is missing, with no default
...
curve( a_map + b_map*(x - xbar) , add=TRUE ) #x is vector of x values; this just plots regression line from formula. add=TRUE means add to scatterplot
...
mu <- link( m4.3 )
#this gives matrix 1000 x 352; default is 1000 samples; 352 columns, one for each case in original data; so we have a posterior distribution
str(mu)
...
mu.mean <- apply( mu , 2 , mean ) #find the mean for all the esimates
mu.HPDI <- apply( mu , 2 , HPDI , prob=0.89 ) #find the HPDI for all the estimates
...
#but in e.g. above, because formula contains reference to post$a and post$b, it is applied to all the rows in post


```

4.4.3.5 Prediction intervals
We got prediction interval for average height,but can get for the actual observed heights. For thsi we need the sd and its uncertainty.
...
Illustrates distinction between uncertainty in parameter values, and uncertainty in sampling process.
...
vcov(m4,3) #looks awful so better if you round it to 3 places as in code 4.45
...
m4.7 <- quap(
  alist(
    T ~ dnorm(mu,sigma),
    mu <- a + B %*% w, #that looks like matrix calculation...
    a ~ dnorm(6,10),
    w ~ dnorm(0,1),
    sigma ~ dexp(1)
  ),
